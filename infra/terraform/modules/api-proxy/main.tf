...
locals {
  service_name = "showsonar-api-proxy-${var.environment}"
  image_url    = "${var.region}-docker.pkg.dev/${var.project_id}/showsonar/api-proxy:${var.image_tag}"
}
...
resource "google_service_account" "api_proxy" {
  ...
  display_name = "ShowSonar API Proxy (${var.environment})"
}
