---
description: Neon Voyager — New Project Bootstrap Reference
---

# 🚀 Neon Voyager — New Project Setup Guide
...
- **Database:** Firestore (with native offline persistence). `Hive` only for local flags.
  - **macOS Hive lock files:** Lock files live in `~/Library/Containers/com.neonvoyager.app/Data/Documents/`. Stale locks crash with `FileSystemException: lock failed (errno = 35)`. `make run-macos` clears them automatically.
